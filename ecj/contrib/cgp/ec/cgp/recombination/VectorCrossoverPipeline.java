...
public class VectorCrossoverPipeline extends BreedingPipeline {
	...
	public static final String V_SUBGRAPH = "subgraph";
	public static final String V_BLOCK = "block";
	public static final String V_UNIFORM = "uniform";
	
	public final static int C_ONEPOINT = 0;
	public final static int C_SUBGRAPH = 1;
	public final static int C_BLOCK = 2;
	public final static int C_UNIFORM = 3;
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		else if (ctype.equalsIgnoreCase(V_SUBGRAPH))
			crossoverType = C_SUBGRAPH;
	...
	{
		AdvancedIntegerVectorSpecies s = (AdvancedIntegerVectorSpecies) state.population.subpops.get(0).species;
		int start = inds.size();
		...
		for (int q = start; q < n + start; /* no increment */) // keep on going until we're filled up
		{
			...
			case C_ONEPOINT:
				...
				break;
			case C_SUBGRAPH:
				p1.subgraphCrossover(state, thread, p2);
				break;
			case C_BLOCK:
				p1.blockCrossover(state, thread, p2, blockSize);
				break;
			case C_UNIFORM:
				s.determineActiveFunctionNodes(p1.activeFunctionNodes, s, p1.genome);
				s.determineActiveFunctionNodes(p2.activeFunctionNodes, s, p2.genome);
				
				Collections.sort(p1.activeFunctionNodes);
				Collections.sort(p2.activeFunctionNodes);
