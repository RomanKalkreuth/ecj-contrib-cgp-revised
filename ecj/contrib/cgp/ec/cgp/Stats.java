 ...
 * information about any new individual found that exceeds the currently tracked
...
public class Stats extends SimpleStatistics {
	...
	public void postEvaluationStatistics(EvolutionState state) {
		...
		for (int i = 0; i < indBefore.length; i++) {
			if (newBest || (best_of_run[i].fitness.betterThan(indBefore[i].fitness)
					&& !best_of_run[i].fitness.equivalentTo(indBefore[i].fitness))) {
				String msg = " NEW BEST: Subpop " + i + ", generation " + state.generation + ", individual: "
		...
		/*
		 * Compute mean fitness for entire population. TODO: generalize to multiple
		 * subpops
		 */
		float total = 0;
